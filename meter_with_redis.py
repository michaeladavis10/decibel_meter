...
def get_path(base, tail, head=""):
    return (
        ...
        if head == ""
        else get_path(head, get_path(base, tail)[1:])
    )
...
def listen(old=0, error_count=0):
    ...
    filename = os.path.join(
        BASE_DIR,
        "data",
        start_day.strftime("%Y"),
        start_day.strftime("%Y%m%d") + ".csv",
    )
    warnings = 0
    infractions = 0
    ...
    with open(filename, "a") as csv_file:
        ...
        while datetime.datetime.now().date() == start_day:
            ...
            try:
                ...
                ## Send to redis queue
                socketio.emit("decibel data", {"data": new}, broadcast=False)
                ...
                if abs(old - new) > 3:
                    ...
                    warnings, infractions = control_led(
                        decibel=new, warnings=warnings, infractions=infractions
                    )
